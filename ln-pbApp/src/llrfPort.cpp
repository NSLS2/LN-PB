...
llrfPort::llrfPort(const char *portName, const char *pSerial, int debug)
...
printf("llrfPort::llrfPort: NUM_PARAMS=%d\n", NUM_PARAMS);
// cout << "llrfPort: NUM_PARAMS=" << NUM_PARAMS << endl;
...
/* Enable (bit 9 to EP SDRAM                            */
    ...
    AERR(createParam("RAM Fill",  asynParamInt32, &P_ram_Filling));
    RAM_Filling_Sts = 0;
...
unsigned
   ...
   for(int i=0; i<DAC_COUNT; ++i) {
	/* SEP 27, 2012 - Change the x-axis of FF & FB tables to counts: 0 ... 512
	   to match the scopes and readback channels ...
	   all the equations in tables have to follow ...
	*/
...
try {
    ...
    err = e = okFrontPanel_OpenBySerial(H, serial.c_str());
    if(e != ok_NoError) {
    ...
    m  = okFrontPanel_GetBoardModel(H);
    AERR(setStringParam(P_model, ModelName(m) ) );
    ...
    if(addr!=0) asynPortDriver::connect(pasynUserSelf); // self is connected to addr 0
...
try {
	...
	{
	strncpy( bitdirname, value, sizeof(bitdirname)-1 );
	// cout << "writeOctet: bitdir:" << value << endl;
	}
    else
    if( function == P_bit_file )
	{
	strncpy( bitfilename, value, sizeof(bitfilename)-1 );
...
try {
    ...
    } else if(function==P_ram_Filling) {

	RAM_Filling_Sts = value;
...
unsigned
        ...
        printf("P_update: TrigOutVal=0x%08x\n", TrigOutVal);

        clock_gettime(CLOCK_REALTIME, &ts[2]);
        T0 = (unsigned int)ts[2].tv_sec;
        AERR(setIntegerParam(P_ram_TS, T0));
        AERR(getIntegerParam(P_ramRdEn, (int *)&RAM_RdEn));
        AERR(getIntegerParam(P_ramWrEn, (int *)&RAM_WrEn));
        AERR(getIntegerParam(P_ram_Filling, (int *)&RAM_Fill));
        AERR(callParamCallbacks());

        if( RAM_RdEn && RAM_WrEn && RAM_Fill ) Qualifier = 1;
        RAM_Fill = 0;
        AERR(setIntegerParam(P_ram_Filling, RAM_Fill));

        TrigSource |= okFrontPanel_IsTriggered(H, EP_STATUS, 0x10)?1:0;
	printf("SDRAM DONE Is Triggered -\n");
        printf("RAM_RdEn=%d, RAM_WrEn=%d, Qualifier=%d\n", RAM_RdEn, RAM_WrEn, Qualifier);
	    ...
	    unlock();
            } // End of if( RAM_RdEn && RAM_WrEn && RAM_Fill )
            else {
            printf("NO QUALIFIER = SDRAM TRIG IGNORED!\n");
            }
 	} // End of if(TrigOutVal)
...
try {
	...
	dval = value * DAC_MAXVALUE; // Val = 0 ... 1
        ...
        throw std::runtime_error("Parameter is readonly");
...
// Nathan, March 20, 2012
	...
	AERR( setIntegerParam( P_WO_EP29, oval[ 9] ) );
	...
	AERR( setIntegerParam( P_WO_EP2e, oval[14] ) );
    ...
    */
    ...
    TrigOutVal = 0;
    TrigOutVal += okFrontPanel_IsTriggered(H, EP_STATUS, STATUS_ADC_DONE)    ? 1<<(STATUS_ADC_DONE-1) : 0;
    TrigOutVal += okFrontPanel_IsTriggered(H, EP_STATUS, STATUS_RAM_DONE)    ? 1<<(STATUS_RAM_DONE-1) : 0;
    TrigOutVal += okFrontPanel_IsTriggered(H, EP_STATUS, STATUS_FBRAMP_DONE) ? 1<<(STATUS_FBRAMP_DONE-1) : 0;
    TrigOutVal += okFrontPanel_IsTriggered(H, EP_STATUS, STATUS_AVAGO2_TRIG) ? 1<<(STATUS_AVAGO2_TRIG-1) : 0;
    TrigOutVal += okFrontPanel_IsTriggered(H, EP_STATUS, STATUS_PPS_EV)      ? 1<<(STATUS_PPS_EV-1) : 0;
    TrigOutVal += okFrontPanel_IsTriggered(H, EP_STATUS, STATUS_RFINH_EV)    ? 1<<(STATUS_RFINH_EV-1) : 0;
    TrigOutVal += okFrontPanel_IsTriggered(H, EP_STATUS, STATUS_ST_EV)       ? 1<<(STATUS_ST_EV-1) : 0;
    TrigOutVal += okFrontPanel_IsTriggered(H, EP_STATUS, STATUS_QUENCH_EV)   ? 1<<(STATUS_QUENCH_EV-1) : 0;
    TrigOutVal += okFrontPanel_IsTriggered(H, EP_STATUS, STATUS_VAC_EV)      ? 1<<(STATUS_VAC_EV-1) : 0;
    TrigOutVal += okFrontPanel_IsTriggered(H, EP_STATUS, STATUS_FFRAMP_DONE) ? 1<<(STATUS_FFRAMP_DONE-1) : 0;
